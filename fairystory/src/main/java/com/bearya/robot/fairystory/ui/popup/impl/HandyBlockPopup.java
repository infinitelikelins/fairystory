...
public class HandyBlockPopup extends AbsBasePopup {
    ...
    private BlockCallback callback = null;
    ...
    private final AdvertiseCallback mAdvertiseCallback = new AdvertiseCallback() {
        ...
        public void onStartSuccess(AdvertiseSettings settingsInEffect) {
            updateBlockMessage(getContext().getString(R.string.blocks_wait_command), R.drawable.handy_block_wait);
        }

        @Override
        public void onStartFailure(int errorCode) {
            updateBlockMessage(getContext().getString(R.string.blocks_start_fail), R.drawable.handy_block_error);
        }
    };
        ...
        public void onCharacteristicWriteRequest(BluetoothDevice device, int requestId, BluetoothGattCharacteristic characteristic, boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
            ...
            if (value != null && value.length > 0) {
                if ((value[0] & 0xff) == 0xf8 && (value[value.length - 1] & 0xff) == 0xf9) {
                    ...
                    updateBlockMessage(getContext().getString(R.string.blocks_command_received), R.drawable.handy_block_success);
        ...
        public void onConnectionStateChange(BluetoothDevice device, int status, int newState) {
            if (status == 0) {
                if (newState == BluetoothGatt.STATE_CONNECTED) {
                    updateBlockMessage(getContext().getString(R.string.blocks_link_connected), R.drawable.handy_block_wait);
                } else if (newState == BluetoothGatt.STATE_DISCONNECTED) {
                    updateBlockMessage(getContext().getString(R.string.blocks_link_disconnected), R.drawable.handy_block_error);
                } else {
                    updateBlockMessage(getContext().getString(R.string.blocks_link_status_change), R.drawable.handy_block_wait);
                }
            } else {
                updateBlockMessage(getContext().getString(R.string.blocks_link_fail), R.drawable.handy_block_error);
            }
        }
    };

    public HandyBlockPopup(Context context) {
        super(context);
    }

    @Override
    protected int inflateLayoutId() {
        return R.layout.popup_handy_blocks;
    }
    ...
    protected void onViewInflated() {
        ...
        withClick(R.id.manual, v -> new ImagePreViewDialog(getContext(), new FacePlay(String.valueOf(R.mipmap.handy_block_combine), FaceType.Image)).show(), false);
        textView = findViewById(R.id.blocks_message);
        imageView = findViewById(R.id.blocks_status);
    ...
    protected void onPopupShow() {
        super.onPopupShow();
        MusicUtil.playAssetsAudio("tts/zh/handy_block.mp3");
        openBluetooth();
    }
    ...
    private void openGattService() {
        ...
        if (registerReceiver != null) {
            getContext().unregisterReceiver(registerReceiver);
        ...
        BluetoothManager systemService = (BluetoothManager) getContext().getSystemService(Context.BLUETOOTH_SERVICE);
        mBluetoothGattServer = systemService.openGattServer(getContext(), mBluetoothGattServerCallback);
    ...
    private void openBluetooth() {
        if (getContext().getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)) {
            ...
            if (!BluetoothAdapter.getDefaultAdapter().isEnabled()) {
                updateBlockMessage(getContext().getString(R.string.blocks_starting), R.drawable.handy_block_wait);
                        ...
                        if (intent != null && intent.getAction() != null) {
                            if (intent.getAction().equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                                ...
                                if (state == BluetoothAdapter.STATE_ON) {
                                    ...
                                    updateBlockMessage(getContext().getString(R.string.blocks_closed), R.drawable.handy_block_error);
                                }
                            }
                        }
                    }
                };
                getContext().registerReceiver(registerReceiver, new IntentFilter(BluetoothAdapter.ACTION_STATE_CHANGED));
            } else {
                openGattService();
            }
        } else {
            updateBlockMessage(getContext().getString(R.string.blocks_unsupported), R.drawable.handy_block_error);
        }
    }

    private void updateBlockMessage(final String message, @DrawableRes final int icon) {
    ...
    private void blockInflate(final String blocks) {
        ...
        if (blocks.isEmpty()) {
            updateBlockMessage(getContext().getString(R.string.blocks_command_empty), R.drawable.handy_block_error);
            return;
        }
        ...
        if (actions.length > 0) {
            ...
            getContext().runOnUiThread(() -> {
                if (callback != null) {
                    callback.onBlockDef(cardParentActions);
                }
            });
        } else {
            updateBlockMessage(getContext().getString(R.string.blocks_command_empty), R.drawable.handy_block_error);
