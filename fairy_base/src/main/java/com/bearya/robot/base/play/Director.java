...
public class Director implements MediaPlayer.OnCompletionListener {
    ...
    private String currentEmotion = "hg";
    private String commandEmotion;
    ...
    private void directPlay(PlayData playData) {
        ...
        if (playData != null) {
            ...
            if (!TextUtils.isEmpty(playData.playAction)) {
                BaseApplication.sendAction(playData.playAction);
            }
                ...
                public void run() {
                    switch (facePlay.getFaceType()) {
                        ...
                        case Image:
                            int mipmapId = 0;
                            try {
                                mipmapId = Integer.valueOf(facePlay.getFace());
                            } catch (Exception e) {
                            ...
                            break;
                        case FrameAnimate:
                            int drawable = Integer.valueOf(facePlay.getFace());
                            playAnimation(drawable);
                            break;
                        case Arrays:
                            int array = Integer.valueOf(facePlay.getFace());
...
//                            BaseApplication.getInstance().getHandler().postDelayed(frameAnimationCompleteRunnable, len * FRAME_DURING);
                            break;
                    }
                }
            });
    ...
    public void setThemeEmotion(String emotion, String command) {
        this.currentEmotion = emotion;
        this.commandEmotion = command;
    }

    public void playMovingEmotion() {
        if (!TextUtils.isEmpty(commandEmotion)) {
            BaseApplication.sendAction(commandEmotion);
        }
        playFace(new FacePlay(currentEmotion, FaceType.Lottie));
    }
}
